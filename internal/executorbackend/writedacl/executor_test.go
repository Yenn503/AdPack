...
func TestCapability(t *testing.T) {
	e := &Executor{}
	if e.Capability() != "WRITE_DACL" {
		t.Fatalf("expected WRITE_DACL, got %s", e.Capability())
	}
}

func TestCanExecute_WriteDacl(t *testing.T) {
	e := &Executor{}
	if !e.CanExecute(context.Background(), core.PrivilegeEdge{
		...
		t.Fatal("expected true for WriteDacl")
	}
}

func TestCanExecute_WriteOwner(t *testing.T) {
	e := &Executor{}
	if !e.CanExecute(context.Background(), core.PrivilegeEdge{
		SourcePrincipal: "alice",
		TargetPrincipal: "bob",
		Domain:          "TEST",
		AccessRight:     "WriteOwner",
	}, &core.ADState{}) {
		t.Fatal("expected true for WriteOwner")
	}
}
...
func TestExecute_IdentityObservation(t *testing.T) {
	...
	if obs.Type != "dacl_modified" {
		t.Fatalf("expected dacl_modified, got %s", obs.Type)
	}
}
