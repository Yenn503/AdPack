...
func (db *DB) SaveState(s *core.ADState) error {
	...
	stmtUser, err := tx.Preparex(`INSERT INTO users(username,domain,sam_account_name,sid,enabled,is_admin,is_da,description,source) VALUES(?,?,?,?,?,?,?,?,?)
	...
	for _, u := range s.Users {
		if _, err := stmtUser.Exec(u.Username, u.Domain, u.SAMAccountName, u.SID, boolInt(u.Enabled), boolInt(u.IsAdmin), boolInt(u.IsDA), u.Description, u.Source); err != nil {
			return err
		}
	}
